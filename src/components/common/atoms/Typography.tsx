...
const Typography = (props:{variant?:TypographyVariants, color?:string, light?:boolean, medium?:boolean, bold?:boolean, italic?:boolean, size?:number, style?:any, children?:any}) => {
    let {variant, color, light, medium, bold, italic, size, style, children} = props;
    function validateProps() {
        if ((light && medium) || (light && bold) || (medium && bold)) throw new Error("You can only choose one of the following: Light, Medium, Bold");
    }
    validateProps();
    ...
    return <Text style={[{fontFamily: fontName, color, fontSize: size}, style]}>{children}</Text>
}
